...
fn add_numbers(lines: &Vec<String>, numbers: &mut Vec<Vec<(usize, usize)>>, row: usize, col: &usize) -> u32 {
  let mut sum: u32 = 0;
...
fn add_gears(lines: &Vec<String>, numbers: &mut Vec<Vec<(usize, usize)>>, row: usize, col: &usize) -> (u32, u32) {
  let mut count = 0;
  let mut ratio: u32 = 1;
  for (start_idx, end_idx) in numbers[row].iter_mut() {
    if !(*col+1 < *start_idx || *col-1 > *end_idx) {
